...
#softmax作为激活函数，常用于多分类问题，能够将神经网络的输出转化为概率分布，分布在0-1中间且它们所有概率的和为1
class NET(nn.Module):
...
#我认为本题属于监督学习，因为使用到了有标签的数据集进行训练，batch_size在这串代码中没有明确指定，则默认值应该为1。这种大小的batch_size并不适用于大数据，因为它效率较低，无法高效的处理大批量的数据集，应更改换用batch_size更大的数据集 